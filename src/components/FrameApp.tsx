...
import { ArrowLeft, ExternalLink, X, Heart, Video, ChevronDown, Loader2 } from 'lucide-react';
import {
  useAccount,
  useSendTransaction,
...
const useTransactionStatuses = (transactionHashes: { [projectId: string]: `0x${string}` }) => {
...
export default function FrameApp() {
  ...
  const [transactionHashes, setTransactionHashes] = useState<{ [projectId: string]: `0x${string}` }>({});
  const [transactionErrors, setTransactionErrors] = useState<{ [projectId: string]: string }>({});
      ...
      try {
        ...
        const mappedProjects: UIProject[] = projects.map(project => {
          ...
          const getProblem = (): string => {
            if (isGrantProject && projectDetails.problem) return projectDetails.problem;
            if (grantDescription) {
              const problemPart = grantDescription.split('##').find(part => part.includes('Why it Matters'));
          ...
          const getSolution = (): string => {
            if (isGrantProject && projectDetails.solution) return projectDetails.solution;
            if (grantDescription) {
              const solutionPart = grantDescription.split('##').find(part => part.includes('How it works'));
          ...
          return {
            ...
            videoUrl: projectDetails.links.find(link => link.type === 'youtube')?.url,
            demoUrl: projectDetails.links.find(link => link.type === 'website')?.url || 'https://gap.karmahq.xyz',
            tags: projectDetails.links.map(link => link.type).filter(type => type !== 'website'),
        ...
        console.error('Error fetching projects:', error, { seasonId: currentSeasonId });
      ...
      if (dir === 'right' && currentProject) {
        setLikedProjects(prev =>
          prev.some(p => p.id === currentProject.id)
            ? prev
            : [...prev, { ...currentProject, payoutAddress: currentProject.payoutAddress || '' }]
        );
      }
  ...
  const handleSwitchChain = useCallback(() => {
    ...
    try {
      ...
      console.error('Chain switch failed:', error, { targetChainId: targetChain.id });
      setWalletError(`Failed to switch to ${targetChain.name}. Please try again.`);
    }
  ...
  const sendDonations = useCallback(async () => {
    ...
    for (const project of likedProjects) {
      if (!project.payoutAddress) {
        console.warn('No payout address for project:', project.id, project.title);
      ...
      if (!isAddress(project.payoutAddress)) {
        console.warn('Invalid payout address for project:', project.id, project.payoutAddress);
        setTransactionErrors(prev => ({
          ...prev,
      ...
      try {
        const value = targetChain.id === celo.id ? parseEther('0.1') : parseEther('0.00004');
        ...
        await new Promise<void>((resolve, reject) => {
          sendTransaction(
            ...
            {
              onSuccess: hash => {
                console.log('Transaction sent:', { projectId: project.id, hash });
              ...
              onError: error => {
                console.error('Transaction failed:', { projectId: project.id, error });
                setTransactionErrors(prev => ({
                  ...prev,
      ...
      <div className="mb-4">
        {isConnected ? (
          <div className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-2xl shadow-sm">
            ...
            >
              {isDisconnectPending && <Loader2 size={16} className="mr-2 animate-spin" />}
              Disconnect
            </button>
          </div>
        ) : (
          ...
          >
            {(isConnectPending || isConnecting) && <Loader2 size={16} className="mr-2 animate-spin" />}
            Connect Wallet
          </button>
        )}
        {isConnected && chainId !== targetChain.id && (
          ...
          >
            {isSwitchChainPending && <Loader2 size={16} className="mr-2 animate-spin" />}
      ...
      <div className="mb-4 sm:mb-6 relative z-10">
        ...
        >
          <div className="flex items-center">
            <div className="w-2 h-8 rounded-full mr-3" style={{ backgroundColor: celoColors.gold }} />
            <div>
            ...
            {seasons.map(season => (
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: season.id === currentSeasonId ? celoColors.gold : '#D1D5DB' }}
                />
        ...
        {outOfProjects ? (
          <div className="flex flex-col items-center justify-center w-full h-full bg-gradient-to-br from-green-50 to-gold-50 rounded-2xl shadow-lg text-center animate-fade-in p-4">
            ...
            <p className="text-gray-600 mb-6 px-4">
              You&apos;ve reviewed all projects in {currentSeason?.name ?? 'this program'}
            </p>
            <button
            ...
            {likedProjects.length > 0 && (
              <div className="w-full p-4 bg-white border border-gray-200 rounded-2xl shadow-sm">
                <h3 className="text-lg font-semibold text-gray-800 mb-2">Support Your Liked Projects</h3>
                <p className="text-sm text-gray-600 mb-4">
                  Send 0.1 USD to {likedProjects.length} liked project(s).
                </p>
                <button
                  onClick={sendDonations}
                  disabled={!isConnected || chainId !== targetChain.id || isDonating || isSendTxPending}
                  className="w-full p-4 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-2xl font-semibold shadow-lg hover:from-green-600 hover:to-green-700 transition-all duration-300 disabled:bg-gray-400 flex items-center justify-center"
                >
                ...
                {Object.entries(transactionStatuses).map(([projectId, status]) => (
                  <div key={projectId} className="mt-2 text-sm text-gray-600">
                    {likedProjects.find(p => p.id === projectId)?.title ?? 'Unknown Project'}: {status}
                  </div>
                ))}
                {Object.entries(transactionErrors).map(([projectId, error]) => (
                  <div key={projectId} className="mt-2 text-sm text-red-500">
                    {likedProjects.find(p => p.id === projectId)?.title ?? 'Unknown Project'}: {error}
                  </div>
                ))}
              </div>
            )}
    ...
    {showVideo && currentProject?.videoUrl ? (
      ...
      <div className="mt-6 sm:mt-8 pt-4 border-t border-gray-200">
        <div className="flex justify-between items-center text-sm text-gray-500">
          <span>
            {currentSeason?.name ?? 'Program'} • {filteredProjects.length} projects
          </span>
          <span>
